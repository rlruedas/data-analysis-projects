...
author: "Humphrey Demoulder Pimentel & Reister L. Ruedas & Nathaniel A. Cayco"
date: "6/8/2021"
...
```{r}
#Top 10 Highest Average IQ per Class
iq.per.class = ddply(nlschools, "class", summarize, num.stud = mean(GS), avg.iq = mean(IQ))
iq.per.class = arrange(iq.per.class, desc(avg.iq))
kable(head(iq.per.class, 10))
```


```{r}
a = lm(nlschools$lang ~ nlschools$IQ)
summary(a)
```
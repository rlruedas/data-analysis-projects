---
title: 'Asynchronous Activity: Data Aggregation and Statistical R Commands'
...
* Check if there are missing value in the Dataset 
```{r}
# All data for this dataset are complete meaning it is good for study.
complete.cases(airlines)
```
...
```{r}
ddply(counts, "Date", mutate, percent = number.passengers / sum(airlines$passenger_numbers))
```
